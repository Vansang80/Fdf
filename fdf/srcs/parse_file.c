...
/*   Updated: 2018/03/15 22:46:49 by pasnguye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     check_file(int fd, t_mlx *mlx)
{
    ...
    while ((ret = get_next_line(fd, &line)) > 0)
    {
        if (!(split = ft_strsplit(line, ' ')) && ft_free(&line))
        {
            printf("bad split, line: %d\n", mlx->size.y);
            return (ERROR);
        }
        ft_free(&line);
        if ((mlx->size.x != ft_check_line(split)))
        {
            printf("bad size, line: %d\n", mlx->size.y);
            return (ERROR);
        }
        mlx->size.y += 1;
    }
